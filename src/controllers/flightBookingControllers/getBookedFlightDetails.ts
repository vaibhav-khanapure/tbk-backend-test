...
const getBookedFlightDetails = async (req: Request, res: Response, next: NextFunction) => {
 try {
  ...
  const results = await FlightBookings?.findAll({
   ...
   raw: true,
   attributes: {
    exclude: ["id", "TraceId", "InvoiceId", "created_at", "updated_at", "discount", "markup", "userId", "fareType", "discountUpdatedByStaffId", "tboPassenger", "tboAmount", "tbkAmount", "PNR", "InvoiceNo"]
   }
  });
